<script lang="ts">
	...
	let avgFriendliness = $derived(
		$encounters.length > 0
			? ($encounters.reduce((s, e) => s + e.friendliness, 0) / $encounters.length).toFixed(1)
			: '—'
	);
...
<svelte:head>
	...
	<meta name="description" content="Track your dog walks and predict friendly meets." />
</svelte:head>
...
{#if !$currentUser}
	<div class="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden">
		<div class="absolute inset-0 overflow-hidden pointer-events-none">
			...
			<div class="absolute -bottom-20 -left-20 w-64 h-64 rounded-full bg-accent/5 blur-3xl"></div>
		</div>
		<div class="text-center relative z-10 max-w-md animate-fade-in">
			...
			<button onclick={() => signInWithGoogle()} class="btn btn-lg btn-primary gap-3 rounded-full shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all duration-300 hover:scale-105 w-full max-w-xs" id="btn-sign-in-google">
			...
			<p class="mt-6 text-base-content/30 text-sm">Free to use • No credit card required</p>
		</div>
	</div>
{:else if !$userProfile}
		...
		<div class="w-full max-w-md animate-fade-in">
			...
			<form onsubmit={handleOnboarding} class="glass-card p-6 space-y-5">
				<div class="form-control">
					<label class="label" for="displayName"><span class="label-text font-medium">Your name</span></label>
					<input type="text" name="displayName" id="displayName" placeholder="e.g. Sarah" class="input input-bordered w-full bg-base-200/50 focus:border-primary" required value={$currentUser?.displayName || ''} />
				</div>
				<div class="form-control">
					<label class="label" for="dogName"><span class="label-text font-medium">Your dog's name 🐕</span></label>
					<input type="text" name="dogName" id="dogName" placeholder="e.g. Buddy" class="input input-bordered w-full bg-base-200/50 focus:border-primary" required />
				</div>
				<div class="form-control">
					<label class="label" for="dogBreed"><span class="label-text font-medium">Breed <span class="text-base-content/40">(optional)</span></span></label>
					<input type="text" name="dogBreed" id="dogBreed" placeholder="e.g. Golden Retriever" class="input input-bordered w-full bg-base-200/50 focus:border-primary" />
				</div>
				<button type="submit" class="btn btn-primary w-full rounded-full mt-2" id="btn-save-profile">Let's Go! 🐾</button>
			</form>
		</div>
	</div>
{:else}
	<div class="pb-safe">
		<div class="p-5 pt-8 max-w-lg mx-auto space-y-6">
			<div class="animate-fade-in">
				...
				<h1 class="text-2xl font-bold mt-1">{$userProfile.displayName} & <span class="gradient-text">{$userProfile.dogName}</span></h1>
			</div>

			<div class="grid grid-cols-3 gap-3 animate-fade-in stagger-1">
				<div class="glass-card p-4 text-center"><div class="text-2xl font-bold text-primary">{totalEncounters}</div><div class="text-xs text-base-content/50 mt-1">Meets</div></div>
				<div class="glass-card p-4 text-center"><div class="text-2xl font-bold text-secondary">{uniqueDogs}</div><div class="text-xs text-base-content/50 mt-1">Dogs</div></div>
			...
			{#if recentEncounters.length > 0}
				<div class="animate-fade-in stagger-3">
					<div class="flex items-center justify-between mb-3">
						...
						<a href="/history" class="text-primary text-sm hover:underline">See all →</a>
					</div>
					<div class="space-y-2">
			...
			<div class="glass-card p-4 flex items-center gap-3 animate-fade-in stagger-4">
				<div class="text-2xl">{$userProfile.isPaid ? '👑' : '🆓'}</div>
				<div class="flex-1">
					<div class="font-medium text-sm">{$userProfile.isPaid ? 'Premium' : 'Free'} Plan</div>
					<div class="text-xs text-base-content/40">{$userProfile.isPaid ? 'Predictions from all met users' : 'Predictions from your logs only'}</div>
				</div>
				{#if !$userProfile.isPaid}<a href="/settings" class="btn btn-xs btn-accent rounded-full">Upgrade</a>{/if}
			</div>
		</div>
	</div>
